...
每轮对话先只 append 到 `event-buffer.json`，当 pending 事件满 3 条时，再统一触发一次轻量模型（如 `qwen-turbo`）做批量情绪结算：
...
# 当 pending_events >= 3 时，claim 一批并生成给 qwen-turbo 的 prompt
python scripts/epe_settle.py --state-file <state-file> prepare --threshold 3 --model qwen-turbo

# 轻量模型返回 JSON 后，应用并 ack 这批事件
python scripts/epe_settle.py --state-file <state-file> apply \
  --batch-id <batch-id> \
  --settlement-json '{"deltas":{"valence":0.12},"reasoning":"...","confidence":0.81}'
```

> 异步模式详见 `references/async-settlement.md`。处理成功后 batch 会被 ack，不会无限积累。若失败可 `abort` 回滚到 pending 队列。
...
### epe_buffer.py — 异步事件队列

```
python scripts/epe_buffer.py --state-file <path> <command> [options]
```

| 命令 | 选项 | 说明 |
|------|------|------|
| `append` | `--user-msg ... --agent-reply ...` | 追加一轮对话事件到 `event-buffer.json` |
| `should-settle` | `[--threshold 3]` | 检查 pending 事件数是否达到结算阈值 |
| `claim` | `[--threshold 3]` | claim 一批事件进入 `inflight_batches` |
| `ack` | `--batch-id <id>` | 结算成功后确认并移除一批事件 |
| `requeue` | `--batch-id <id>` | 结算失败后将一批事件放回 pending 队列 |
| `get-batch` | `--batch-id <id>` | 查看某个 inflight batch |
| `dump` | — | 导出当前 pending 事件 |
| `stats` | — | 查看队列统计信息 |

### epe_settle.py — 异步结算辅助器

```
python scripts/epe_settle.py --state-file <path> <command> [options]
```

| 命令 | 选项 | 说明 |
|------|------|------|
| `prepare` | `[--threshold 3] [--model qwen-turbo]` | claim 一批事件并生成给轻量模型的结算 prompt |
| `apply` | `--batch-id <id> --settlement-json '{...}'` | 应用轻量模型返回的 delta，并 ack 这批事件 |
| `abort` | `--batch-id <id>` | 结算失败时回滚这批事件到 pending 队列 |
